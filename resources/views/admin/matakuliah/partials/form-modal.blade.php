<div class="modal fade" id="matakuliah-modal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered mw-650px">
        <div class="modal-content">
            <div class="modal-header" id="kt_modal_add_user_header">
                <h2 class="fw-bold">{{ $action === 'edit'? 'Edit' : "Tambah" }} Matakuliah</h2>
                <div class="btn btn-icon btn-sm btn-active-icon-primary" modal-action="close">
                    <i class="ki-duotone ki-cross fs-1">
                        <span class="path1"></span>
                        <span class="path2"></span>
                    </i>
                </div>
            </div>
            <div class="modal-body px-5 my-7">
                <form id="user-form" class="form" action="{{ $route }}" modal-action="{{ $action }}"
                    enctype="multipart/form-data">
                    @if($action == 'edit')
                    @method('PUT')
                    @endif
                    <div class="d-flex flex-column scroll-y px-5 px-lg-10">
                        <div class="fv-row mb-7">
                            <label class="required fw-semibold fs-6 mb-2">Kode Matakuliah</label>
                            <input type="text" name="kode_matakuliah"
                                class="form-control form-control-solid mb-3 mb-lg-0"
                                placeholder="Masukkan Kode Matakuliah (contoh: MK001)" autofocus
                                value="{{ $matakuliah->kode_matakuliah ?? old('kode_matakuliah') }}" />
                        </div>
                        <div class="fv-row mb-7">
                            <label class="required fw-semibold fs-6 mb-2">Nama Matakuliah</label>
                            <input type="text" name="nama_matakuliah"
                                class="form-control form-control-solid mb-3 mb-lg-0"
                                placeholder="Masukkan Nama Matakuliah"
                                value="{{ $matakuliah->nama_matakuliah ?? old('nama_matakuliah') }}" />
                        </div>
                        <div class="fv-row mb-7">
                            <label class="required fw-semibold fs-6 mb-2">Deskripsi</label>
                            <textarea name="deskripsi" class="form-control form-control-solid mb-3 mb-lg-0"
                                placeholder="Masukkan Deskripsi Matakuliah">{{ $matakuliah->deskripsi ?? old('deskripsi') }}</textarea>
                        </div>
                        <div class="fv-row mb-7">
                            <label class="required fw-semibold fs-6 mb-2">SKS</label>
                            <input type="number" name="sks" class="form-control form-control-solid mb-3 mb-lg-0"
                                placeholder="Masukkan SKS" min="1" max="6"
                                value="{{ $matakuliah->sks ?? old('sks') }}" />
                        </div>
                    </div>
                    <div class="text-center pt-10">
                        <button type="reset" class="btn btn-light me-3">Reset</button>
                        <button type="submit" class="btn btn-primary">
                            <span class="indicator-label">Submit</span>
                        </button>
                    </div>
                </form>
            </div>
        </div>
    </div>
</div>
<script src="{{ asset('assets/js/custom-modal.js') }}"></script>